...
const CurrentGroup = (integrations: IntegrationType[] = []) => createParamDecorator({
  ...
  required: true,
  value: (({ request }: Action) => getGroupFromRequest(request, integrations))
});

export async function getGroupFromRequest (req: any, integrations: IntegrationType[] = []): Promise<Group> {
